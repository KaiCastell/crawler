    ...
    if(ctx.channel.name == 'crawler'): #only respond to the command if it was in the crawler channel
        match(arg):
            ...
            case "testg":
                ...
                print("success")
            case "board":
                # board = gameLoop.board(playerList.players, "random")
...
class ClassSelectDropdown(discord.ui.Select): # this is the dropdown selection, but not the view
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(playerList.addFromDropdown(name, className), ephemeral=True)
